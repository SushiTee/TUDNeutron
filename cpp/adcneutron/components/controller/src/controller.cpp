...
void Controller::sendDmaData() {
    ...
    while (!threadQuit)
    {
        if (!mDma->hasEnoughData()) {
            std::this_thread::sleep_for(1ms);
            continue;
        }

        if (mDma->full()) {
            LogOut << "Fifo is full " << std::to_string(mDma->getID()) << std::endl;
            sendData(MessageType::FIFO_FULL, "{\"status\":\"Error\",\"msg\":\"Fifo is full.\",\"dma\":" + std::to_string(mDma->getID()) + "}");
            return;
        }

        size = mDma->writeSize();
        offset = mDma->readAddress();
        payloadSize = static_cast<uint16_t>(size / mDma->getWordLength()); // 0 means max!
        mDma->setReadAddress(size);

        // write header
        conv.num = static_cast<uint16_t>(payloadSize);
        message[0] = mDma->getID();
        message[1] = conv.bytes[0];
        message[2] = conv.bytes[1];

        sendDataImpl(reinterpret_cast<const std::byte*>(message.data()), reinterpret_cast<const std::byte*>(mDma->memoryMap() + offset), size * 4);
    }
}
...
bool Controller::handleData(kn::buffer<BUFFER_SIZE> &buff, MessageType type, size_t size) {
    ...
    case MessageType::START_DMA: {
        ...
        uint8_t sensors = static_cast<uint8_t>(buff[0]);
        if (sensors == 0u) {
            LogErr << "Error no DMA should be activated" << std::endl;
            networkOK = sendData(type, "{\"status\":\"Error\",\"msg\":\"Error no Sensor should be activated.\"}");
            break;
        }
        ...
        auto dma = std::make_unique<dma::Dma>(0, mMem);
        if (dma->hasError()) {
        ...
        // do busy wait until Dma is started
        using namespace std::chrono_literals;
        while (true) {
            if (mDma->isRunning()) {
                break;
        ...
        gpio::SensorController::activateSpecific(sensors);
        break;
    }
...
bool Controller::isSocketValid(kn::socket_status status) const {
    if (status == kn::socket_status::cleanly_disconnected) {
        ...
        LogOut << "disconnected! " << get_error_code() << " " << strerror(get_error_code()) << std::endl;
        return false;
    }
    return true;
}

bool Controller::dmaExists() const {
    if (mDma != nullptr) {
        return true;
    }
    return false;
